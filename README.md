...
참가자 등록 화면에서 PNG, JPG, WebP 행사 배경 이미지를 선택할 수 있습니다. 이미지는 브라우저나 외부 서버가 아니라 앱의 로컬 저장소에 보관되므로 인터넷 연결 없이 유지됩니다. 추첨 화면은 일반 모니터, 4K 디스플레이, 울트라와이드 행사장 화면 크기에 맞춰 번호판을 균일하게 확대·축소합니다. `F11` 키로 전체 화면을 켜거나 끌 수 있습니다.
